...
public class LiquidClassification extends PApplet {
	...
	float curMeasurements[] = new float[3]; // top 3 slopes


	// for machine learning
	String[] classNames = {"No Liquid", "A", "B", "C", "D"};
	...
	public void draw(){
		...
		float size = 88;
	      ...
	      // get top n slopes as machine learning features
	      
	      ArrayList<Float> dataToSort = new ArrayList<Float> (Arrays.asList((Float[])slopes));
	      
	      Collections.sort(dataToSort);
	      
	      for(int i = 0; i < curMeasurements.length; i++) {
	    	  curMeasurements[i] = dataToSort.get(i);
	      }
	      
	      float curMeasurementsDraw[] = new float [curMeasurements.length];
	      
	      for(int i = 0; i < curMeasurements.length; i++) {
	    	  curMeasurementsDraw[i] = map(curMeasurements[i], 4096, 0, height/2, 0);
	      }
	      
	      for(int i = 0; i < curMeasurements.length; i++) {
	    	  fill(0, 255, 0);
	    	  rect(100 + i*100, height - 10, 80, curMeasurementsDraw[i]);
	      }
