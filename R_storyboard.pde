...
void draw() {
  Client thisClient = myServer.available();
      ...
      textSize(20);
      textAlign(LEFT);
      text("FPS: "+fpsIndicator, width/2 - 250, 20);
      text("Threshold: "+dirivativeThreshold, width/2 - 250, 40);
      text("Time Hysteresis: "+timeThreshold, width/2 - 250, 60);
      ...
      noStroke();
      fill(150);
      rect(0, yOffset + 300 , width/2, 255);
      
      // draw threshold
      stroke(255, 200, 0);
      strokeWeight(1);
      line(0, dirivativeThreshold + yOffset + 300 + 255/2, width/2, dirivativeThreshold+yOffset + 300 + 255/2);
      line(0, -dirivativeThreshold + yOffset + 300 + 255/2, width/2, -dirivativeThreshold+yOffset + 300 + 255/2);
      strokeWeight(3);
      stroke(30,144,255);
      for(int i=1; i<w; i++) {
      ...
      image(img_closed, width/2, 0, width, height/2);
      }
  
      if (abs(currDirivative) > dirivativeThreshold ){
        if (currValue > dirivativeThreshold) {
        image(img_opened, width/2, 0, width, height/2);
      } else {
        image(img_closed, width/2, 0, width, height/2);
      }
  }
